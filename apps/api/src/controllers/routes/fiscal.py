...
class FiscalService:
    ...
    def generate_nfe(order_id):
        ...
        nfe_number = (
            f"NFe{datetime.now().strftime('%Y%m%d')}{str(uuid.uuid4().hex[:8]).upper()}"
        )
    ...
    def generate_nfse(order_id):
        ...
        nfse_data = {
            ...
            "totals": {"subtotal": subtotal, "iss": iss, "total": total_with_taxes},
            "emission_date": datetime.now().isoformat(),
            "status": "emitted",
        }
...
def generate_nfe(order_id):
    ...
    try:
        ...
        return jsonify({"message": "NFe gerada com sucesso", "nfe": nfe_data}), 201
...
def generate_nfse(order_id):
    ...
    try:
        ...
        return jsonify({"message": "NFSe gerada com sucesso", "nfse": nfse_data}), 201
