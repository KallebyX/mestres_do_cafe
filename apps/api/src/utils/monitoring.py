...
class MetricsCollector:
    ...
    def get_application_metrics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            avg_response_time = 0
            if recent_requests:
                avg_response_time = sum(r["duration"] for r in recent_requests) / len(
                    recent_requests
                )
...
class AlertManager:
    ...
    def __init__(self):
        self.alert_thresholds = {
            "cpu_percent": 95.0,  # Valor realista para produção
            "memory_percent": 90.0,  # Valor realista para produção
            "disk_percent": 95.0,
            "error_rate": 10.0,  # 10% de erro (mais tolerante)
            "avg_response_time": 5.0,  # 5 segundos (mais tolerante)
        }
    ...
    def check_alerts(self):
        ...
        try:
            ...
            # Registra alertas (apenas logs simplificados para reduzir overhead)
            for alert in alerts:
                alert["timestamp"] = datetime.now().isoformat()
                self.alert_history.append(alert)

                # Log simplificado do alerta
                logger = StructuredLogger("alerts")
                logger.logger.warning(
                    f"{alert['type']}: {alert['value']:.1f} > {alert['threshold']:.1f} ({alert['severity']})"
                )
...
def init_monitoring(app):
    ...
    def after_request(response):
        ...
        if hasattr(g, "start_time"):
            ...
            metrics_collector.record_request_time(
                endpoint, request.method, duration, response.status_code
            )
    ...
    # Agendamento de verificação de alertas (executar a cada 60 segundos)
    import threading
    ...
    def check_alerts_periodically():
        while True:
            try:
                ...
                time.sleep(60)  # Reduzido overhead: de 30s para 60s
            except Exception as e:
                app.logger.error(f"Error in periodic alert check: {e}")
                time.sleep(120)  # Espera mais em caso de erro
...
def get_health_status() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "uptime": time.time()
            - getattr(get_health_status, "_start_time", time.time()),
        }
