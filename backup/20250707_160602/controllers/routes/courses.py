...
from src.models.database import db, Course, CoursePurchase, User
...
def create_course():
    ...
    try:
        ...
        course.start_date = datetime.fromisoformat(data['start_date']) if data.get('start_date') else None
        course.end_date = datetime.fromisoformat(data['end_date']) if data.get('end_date') else None
...
def buy_course(course_id):
    ...
    existing = CoursePurchase.query.filter_by(user_id=user_id, course_id=course_id).first()
    if existing:
