-- ===============================================
-- 🔒 CORREÇÃO COMPLETA DE SEGURANÇA SUPABASE V4
-- Sistema Mestres do Café - Database Linter Fixes
-- VERSÃO FINAL (corrige conflito de nomes)
-- ===============================================

-- Função auxiliar para verificar se coluna existe (CORRIGIDA)
...
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = current_table) THEN
            ...
            END IF;
        ELSE
            RAISE NOTICE 'Tabela % não existe - pulando', current_table;
        END IF;
    END LOOP;
END $$;
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '🏆 CORREÇÕES DE SEGURANÇA V4 APLICADAS COM SUCESSO!';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '✅ Script final executado sem conflitos de nomes';
    RAISE NOTICE '';
